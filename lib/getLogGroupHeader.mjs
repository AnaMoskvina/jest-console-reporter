...
const statusByType = {
  error: chalk.supportsColor ? chalk.reset.bold.red(` ${ERROR_TEXT}`.padEnd(7)) : ERROR_TEXT,
  warn: chalk.supportsColor ? chalk.reset.bold.yellow(` ${WARN_TEXT}`.padEnd(7)) : WARN_TEXT,
  log: chalk.supportsColor ? chalk.reset.bold.cyan(` ${LOG_TEXT}`.padEnd(7)) : LOG_TEXT,
  info: chalk.supportsColor ? chalk.reset.bold.blue(` ${INFO_TEXT}`.padEnd(7)) : INFO_TEXT,
  debug: chalk.supportsColor ? chalk.reset.bold.magenta(` ${DEBUG_TEXT}`.padEnd(7)) : DEBUG_TEXT,
}
...
export const getLogGroupHeader = (type, key, count, testPaths) => {
  const status = statusByType[type] || type.toUpperCase()
  return `${status} ${formatCount(count)} ${formatMessage(key)} ${testPaths}`
}

export const getSkippedHeader = (count) => {
